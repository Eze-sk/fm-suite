        ...
        const matchesTags =
          tags.length > 0
            ? tags.every((tag) => {
                ...
                const matchLanguage = p.languages.some(
                  (lang) => lang.toLowerCase() === tag,
                )
  ...
  return (
    <Box flexDirection="column">
      {STATUS_MESSAGES[status] ? (
        <Box marginBottom={5}>
          <LoaderCard message={STATUS_MESSAGES[status]} isCentered={false} />
        </Box>
      ) : (
        <>
          {search.length > 0 ? (
            ...
            <Box marginBottom={3}>
              <Text bold>
                No challenges were found matching{' '}
                <Text color={theme.info}>"{query}"</Text>
              </Text>
            </Box>
          )}
        </>
